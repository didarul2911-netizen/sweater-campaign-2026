...
print(f"Loaded {len(territories)} territories, {len(region_map)} regions, {len(zones)} zones.")
...
<body class="min-h-screen flex flex-col bg-slate-100 text-slate-800 antialiased">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div class="flex items-center justify-between sm:justify-start gap-2.5">
                    ...
                    <div class="hidden sm:flex items-center gap-2">
                        <div id="cloud-status-indicator" class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200 flex items-center gap-1.5" title="Cloud Database Connection Status">
                            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                            <span id="cloud-status-text">Cloud Sync Ready</span>
                        </div>
    ...
    <main class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-6 flex-1 w-full flex flex-col gap-4 sm:gap-6">
        ...
        <!-- =================================================================== -->
        <!-- VIEW 1: REGIONAL LOGIN (SELECTION VIEW) -->
        <!-- =================================================================== -->
...
###ZONE_OPTIONS###                        </select>
                    ...
                    <div id="unlock-btn-container" class="hidden">
                        <button onclick="unlockRegion()" class="w-full py-3 bg-orange-500 hover:bg-orange-600 text-white font-black text-sm rounded-xl shadow-lg shadow-orange-500/25 transition flex items-center justify-center gap-2 active:scale-98">
                            ...
                            <span>Login to Territory Portal</span>
                        </button>
                    </div>
                ...
                <div class="lg:col-span-9 space-y-4 sm:space-y-5">
                    ...
                    <div id="active-territory-banner-card" class="sticky-territory-banner bg-slate-900/95 backdrop-blur-md border border-slate-800 rounded-2xl p-3 sm:px-5 sm:py-3.5 flex items-center justify-between shadow-xl text-white">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <button onclick="saveCurrentTerritoryClick()" class="px-3 sm:px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-black rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-500/20 transition active:scale-95">
                ...
                <div class="w-full sm:w-1/2 space-y-3">
                    ...
                    <div class="bg-slate-50 rounded-2xl p-3 border border-slate-200 space-y-2 text-xs">
                        <div class="flex justify-between border-b border-slate-200/60 pb-1.5">
                            ...
                            <span class="font-bold text-slate-800">Richman / Lubnan</span>
                        </div>
                        <div class="flex justify-between border-b border-slate-200/60 pb-1.5">
                            <span class="text-slate-500">Available Sizes:</span>
                            <span id="lightbox-sizes" class="font-black text-orange-600">S, M, L, XL, XXL</span>
                        </div>
                    </div>

                    <button onclick="closeImageLightbox()" class="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl transition shadow">
                        Close Preview
                    </button>
                </div>
            </div>
        ...
        <div class="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden" onclick="event.stopPropagation()">
            <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-xl bg-orange-500 text-white flex items-center justify-center font-bold text-sm shadow-sm">
                ...
                <button onclick="closeCatalogModal()" class="w-8 h-8 rounded-full bg-white border border-slate-300 text-slate-600 hover:bg-slate-100 flex items-center justify-center transition">
            ...
            <div class="p-4 sm:p-6 overflow-y-auto custom-scrollbar space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                    <div onclick="openImageLightbox('01')" class="bg-slate-50 border border-slate-200 hover:border-orange-400 rounded-2xl p-2.5 space-y-2 cursor-pointer transition group shadow-sm">
                        <div class="aspect-[3/4] bg-white rounded-xl overflow-hidden border border-slate-200 relative">
                            <img src="###B64_01###" onerror="this.src='Image/01 (Men).jpeg'" alt="01" class="w-full h-full object-cover">
                            <span class="absolute top-1.5 left-1.5 bg-slate-900/90 text-white text-[10px] font-black px-2 py-0.5 rounded-md">01</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold uppercase text-orange-700 bg-orange-50 px-1.5 py-0.5 rounded">Men's</span>
                            <h4 class="text-xs font-black text-slate-900 mt-1 leading-tight">V-Neck (Grey)</h4>
                            <p class="text-[10px] text-slate-500">Sizes: S - XXL</p>
                        </div>
                    </div>
                    ...
                    <div onclick="openImageLightbox('02')" class="bg-slate-50 border border-slate-200 hover:border-orange-400 rounded-2xl p-2.5 space-y-2 cursor-pointer transition group shadow-sm">
                        <div class="aspect-[3/4] bg-white rounded-xl overflow-hidden border border-slate-200 relative">
                            <img src="###B64_02###" onerror="this.src='Image/02 (Men).jpeg'" alt="02" class="w-full h-full object-cover">
                            <span class="absolute top-1.5 left-1.5 bg-slate-900/90 text-white text-[10px] font-black px-2 py-0.5 rounded-md">02</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold uppercase text-orange-700 bg-orange-50 px-1.5 py-0.5 rounded">Men's</span>
                            <h4 class="text-xs font-black text-slate-900 mt-1 leading-tight">V-Neck (Navy)</h4>
                            <p class="text-[10px] text-slate-500">Sizes: S - XXL</p>
                        </div>
                    </div>
                    ...
                    <div onclick="openImageLightbox('03')" class="bg-slate-50 border border-slate-200 hover:border-orange-400 rounded-2xl p-2.5 space-y-2 cursor-pointer transition group shadow-sm">
                        <div class="aspect-[3/4] bg-white rounded-xl overflow-hidden border border-slate-200 relative">
                            <img src="###B64_03###" onerror="this.src='Image/03 (Men).jpeg'" alt="03" class="w-full h-full object-cover">
                            <span class="absolute top-1.5 left-1.5 bg-slate-900/90 text-white text-[10px] font-black px-2 py-0.5 rounded-md">03</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold uppercase text-orange-700 bg-orange-50 px-1.5 py-0.5 rounded">Men's</span>
                            <h4 class="text-xs font-black text-slate-900 mt-1 leading-tight">V-Neck (Cream)</h4>
                            <p class="text-[10px] text-slate-500">Sizes: S - XXL</p>
                        </div>
                    </div>
                    ...
                    <div onclick="openImageLightbox('04')" class="bg-slate-50 border border-slate-200 hover:border-orange-400 rounded-2xl p-2.5 space-y-2 cursor-pointer transition group shadow-sm">
                        <div class="aspect-[3/4] bg-white rounded-xl overflow-hidden border border-slate-200 relative">
                            <img src="###B64_04###" onerror="this.src='Image/04 (Female).jpeg'" alt="04" class="w-full h-full object-cover">
                            <span class="absolute top-1.5 left-1.5 bg-slate-900/90 text-white text-[10px] font-black px-2 py-0.5 rounded-md">04</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold uppercase text-purple-700 bg-purple-50 px-1.5 py-0.5 rounded">Women's</span>
                            <h4 class="text-xs font-black text-slate-900 mt-1 leading-tight">Short Cardigan</h4>
                            <p class="text-[10px] text-slate-500">Sizes: XS - XL</p>
                        </div>
                    </div>
                    ...
                    <div onclick="openImageLightbox('05')" class="bg-slate-50 border border-slate-200 hover:border-orange-400 rounded-2xl p-2.5 space-y-2 cursor-pointer transition group shadow-sm col-span-2 sm:col-span-1">
                        <div class="aspect-[3/4] bg-white rounded-xl overflow-hidden border border-slate-200 relative">
                            <img src="###B64_05###" onerror="this.src='Image/05 (Female).jpeg'" alt="05" class="w-full h-full object-cover">
                            <span class="absolute top-1.5 left-1.5 bg-slate-900/90 text-white text-[10px] font-black px-2 py-0.5 rounded-md">05</span>
                        </div>
                        <div>
                            <span class="text-[9px] font-bold uppercase text-purple-700 bg-purple-50 px-1.5 py-0.5 rounded">Women's</span>
                            <h4 class="text-xs font-black text-slate-900 mt-1 leading-tight">Semi Long Cardigan</h4>
                            <p class="text-[10px] text-slate-500">Sizes: S - XXL</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-4 sm:p-6 overflow-y-auto custom-scrollbar space-y-6 flex-1 bg-slate-50/50">
                ...
                <div id="admin-auth-view" class="max-w-sm mx-auto py-10 text-center space-y-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                    ...
                    <div class="space-y-3">
                        ...
                        <button onclick="verifyAdminPassword()" class="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-black shadow transition active:scale-98">
                            Unlock Dashboard
                        </button>
                ...
                <div id="admin-dashboard-view" class="hidden space-y-6">
                    ...
                    <div class="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3">
                        ...
                        <div class="flex items-center gap-2 self-start md:self-auto">
                            <div class="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl text-xs">
                                ...
                                <button id="toggle-global-access-btn" onclick="toggleGlobalSubmissionsAccess()" class="font-bold text-emerald-600 hover:underline">
                                    Open
                                </button>
                            </div>
                            <button onclick="logoutAdmin()" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition">
                    ...
                    <div id="admin-cloud-settings-box" class="hidden bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 rounded-3xl border border-slate-700 shadow-xl space-y-4">
                        <div class="flex items-center justify-between border-b border-slate-700 pb-3">
                            ...
                            <button onclick="toggleCloudSettings()" class="text-slate-400 hover:text-white text-xs">
                                <i class="fa-solid fa-xmark text-sm"></i>
                            </button>
                        </div>
                        ...
                        <div class="space-y-2">
                            <label class="block text-xs font-bold text-slate-300">
                                Google Apps Script Web App URL (Google Sheets API):
                            </label>
                            <div class="flex gap-2">
                                ...
                                <button onclick="saveCloudUrlSetting()" class="px-4 py-2 bg-orange-500 hover:bg-orange-400 text-slate-950 text-xs font-black rounded-xl transition">
                                    Save URL
                                </button>
                            </div>
                            <p class="text-[11px] text-slate-400 leading-relaxed">
                                💡 <strong>How it works:</strong> Paste your Google Apps Script Web App URL here. Every submission from any field phone will stream directly into your Google Sheet in Google Drive in real-time! The script template is available in <code class="text-orange-300 bg-slate-950 px-1 py-0.5 rounded">Google_Apps_Script_Backend.js</code>.
                            </p>
                        </div>

                        <div class="pt-2 border-t border-slate-800 flex flex-wrap items-center gap-2">
                            <button onclick="downloadAllDataJson()" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-xl text-xs font-bold text-slate-200">
                                <i class="fa-solid fa-download"></i> Backup JSON
                            </button>
                            <label class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-xl text-xs font-bold text-slate-200 cursor-pointer">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                        ...
                        <div class="bg-white border border-slate-200 rounded-3xl p-4 sm:p-5 shadow-sm space-y-2">
                            <div class="flex items-center justify-between text-slate-400">
                                <span class="text-[10px] sm:text-xs font-black uppercase tracking-wider">Total Territories</span>
                                <i class="fa-solid fa-map-location-dot text-base text-slate-400"></i>
                            </div>
                            <div class="flex items-baseline gap-1.5">
                                <span id="kpi-total-territories" class="text-2xl sm:text-3xl font-black text-slate-900">1,856</span>
                                <span class="text-[11px] font-bold text-slate-500">Territories</span>
                            </div>
                        ...
                        <div class="bg-white border-2 border-emerald-500/40 rounded-3xl p-4 sm:p-5 shadow-sm space-y-2">
                            <div class="flex items-center justify-between text-emerald-600">
                                <span class="text-[10px] sm:text-xs font-black uppercase tracking-wider">Completed</span>
                                <i class="fa-solid fa-circle-check text-base"></i>
                            </div>
                            <div class="flex items-baseline gap-1.5">
                                <span id="kpi-completed-count" class="text-2xl sm:text-3xl font-black text-emerald-600">0</span>
                                <span id="kpi-completed-pct" class="text-xs font-black text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded-md">0%</span>
                            </div>
                            <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                <div id="kpi-completed-bar" class="bg-emerald-500 h-full rounded-full transition-all duration-500" style="width: 0%"></div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white border border-amber-300 rounded-3xl p-4 sm:p-5 shadow-sm space-y-2">
                            <div class="flex items-center justify-between text-amber-600">
                                <span class="text-[10px] sm:text-xs font-black uppercase tracking-wider">In Progress</span>
                                <i class="fa-solid fa-spinner text-base"></i>
                            </div>
                            <div class="flex items-baseline gap-1.5">
                                <span id="kpi-inprogress-count" class="text-2xl sm:text-3xl font-black text-amber-600">0</span>
                                <span id="kpi-inprogress-pct" class="text-xs font-black text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded-md">0%</span>
                            </div>
                            <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                <div id="kpi-inprogress-bar" class="bg-amber-500 h-full rounded-full transition-all duration-500" style="width: 0%"></div>
                            </div>
                        </div>

                        <!-- Card 4: Pending / Remaining -->
                        <div class="bg-white border border-slate-200 rounded-3xl p-4 sm:p-5 shadow-sm space-y-2">
                            <div class="flex items-center justify-between text-slate-400">
                                <span class="text-[10px] sm:text-xs font-black uppercase tracking-wider">Remaining</span>
                                <i class="fa-solid fa-clock text-base text-slate-400"></i>
                            </div>
                            <div class="flex items-baseline gap-1.5">
                                <span id="kpi-pending-count" class="text-2xl sm:text-3xl font-black text-slate-700">1,856</span>
                                <span id="kpi-pending-pct" class="text-xs font-black text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded-md">100%</span>
                            </div>
                            <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                <div id="kpi-pending-bar" class="bg-slate-400 h-full rounded-full transition-all duration-500" style="width: 100%"></div>
                            </div>
                        </div>
                    ...
                    <!-- ============================================== -->
                    <!-- 2. ZONE-WISE PROGRESS BREAKDOWN (35 ZONES)     -->
                    <!-- ============================================== -->
                    <div class="bg-white rounded-3xl border border-slate-200 p-4 sm:p-5 shadow-sm space-y-3">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                            <div>
                                <h4 class="text-xs sm:text-sm font-black uppercase tracking-wider text-slate-900">
                                    Zone-Wise Completion Progress (35 Zones)
                                </h4>
                                <p class="text-[10px] sm:text-xs text-slate-500">Live submission status across all pharmaceutical sales zones</p>
                            </div>
                            <span id="zone-summary-badge" class="text-xs font-black px-2.5 py-1 bg-orange-100 text-orange-800 rounded-xl self-start sm:self-auto">
                                0 / 35 Zones Finished
                            </span>
                        </div>
                        ...
                        <div class="overflow-x-auto max-h-[300px] overflow-y-auto custom-scrollbar">
                            <table class="w-full text-xs text-left border-collapse">
                                <thead class="bg-slate-100 text-slate-700 uppercase text-[10px] sticky top-0 border-b border-slate-200">
                                    <tr>
                                        <th class="p-2.5">Zone Name</th>
                                        <th class="p-2.5 text-center">Regions</th>
                                        <th class="p-2.5 text-center">Territories</th>
                                        <th class="p-2.5 text-center">Done</th>
                                        <th class="p-2.5 text-center">Remaining</th>
                                        <th class="p-2.5">Progress Bar</th>
                                        <th class="p-2.5 text-right">Status</th>
                                    </tr>
                                </thead>
                                <tbody id="admin-zone-table-body" class="divide-y divide-slate-100 font-semibold">
                                </tbody>
                            </table>
                        </div>
                    </div>
                    ...
                    <!-- ============================================== -->
                    <!-- 3. PRODUCTION REQUIREMENT BREAKDOWN MATRIX     -->
                    <!-- ============================================== -->
                    <div class="bg-white rounded-3xl border border-slate-200 p-4 sm:p-5 shadow-sm space-y-3">
                        <div class="border-b border-slate-100 pb-3">
                            <h4 class="text-xs sm:text-sm font-black uppercase tracking-wider text-slate-900">
                                Live Production Requirement Breakdown by Size
                            </h4>
                            <p class="text-[10px] sm:text-xs text-slate-500">Exact sweater counts for Lubnan Trade Consortium Ltd. order generation</p>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-xs text-center border-collapse">
                                <thead class="bg-slate-900 text-white text-[11px] font-bold">
                                    <tr>
                                        <th class="p-2.5 text-left">Item Code & Name</th>
                                        <th class="p-2.5">XS</th><th class="p-2.5">S</th><th class="p-2.5">M</th><th class="p-2.5">L</th><th class="p-2.5">XL</th><th class="p-2.5">XXL</th>
                                        <th class="p-2.5 bg-orange-600 text-white">Total</th>
                                    </tr>
                                </thead>
                                <tbody id="admin-matrix-body" class="divide-y divide-slate-100 font-semibold">
                                </tbody>
                            </table>
                        </div>
                    </div>
                    ...
                    <!-- ============================================== -->
                    <!-- 4. ALL 252 REGIONS TABLE WITH FILTERS          -->
                    <!-- ============================================== -->
                    <div class="bg-white rounded-3xl border border-slate-200 p-4 sm:p-5 shadow-sm space-y-3">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                            <div>
                                <h4 class="text-xs sm:text-sm font-black uppercase tracking-wider text-slate-900">All 252 Regions Status & Actions</h4>
                                <p class="text-[10px] sm:text-xs text-slate-500">Manage individual regional submissions and lock states</p>
                            </div>
                        ...
                        <div class="overflow-x-auto max-h-[360px] overflow-y-auto custom-scrollbar">
                            <table class="w-full text-xs text-left border-collapse">
                                <thead class="bg-slate-100 text-slate-600 uppercase text-[10px] sticky top-0 border-b border-slate-200">
                                    <tr>
                                        <th class="p-2.5">SAP Code</th>
                                        <th class="p-2.5">Region</th>
                                        <th class="p-2.5">Zone</th>
                                        <th class="p-2.5">Regional Head</th>
                                        <th class="p-2.5">Progress</th>
                                        <th class="p-2.5">Lock State</th>
                                        <th class="p-2.5 text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody id="admin-regions-table-body" class="divide-y divide-slate-100 font-medium">
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            // Pull latest cloud data silently in the background on load
            pullCloudData(false);
        ...
        function unlockRegion(bypassCode = null, isRestoringSession = false) {
            ...
            document.getElementById('banner-rh').textContent = `${r.region_name} Region (${r.regional_head})`;
        ...
        function saveCurrentTerritoryClick() {
            ...
            if (terrCode && t) {
                syncTerritoryToCloud(terrCode, store[terrCode], {
                    ...
                    regional_head: r.regional_head,
                    territory_name: t.territory_name
                });
            }

            showToast(`✅ ${name} saved & synced!`);
        }
        ...
        function updateCloudStatusBadge() {
            ...
            if (badgeText) {
                if (cloudApiUrl) {
                    badgeText.textContent = "Google Drive Connected";
                } else {
                    badgeText.textContent = "Cloud Sync Ready";
                }
            }
        ...
        function saveCloudUrlSetting() {
            ...
            updateCloudStatusBadge();
            alert('✅ Cloud API URL saved! Pulling latest data...');
            pullCloudData(true);
        }

        async function syncTerritoryToCloud(terrCode, terrData, meta) {
            ...
            try {
                ...
                console.warn("[Cloud Sync] Background sync note:", err);
            }
        }
        ...
        async function pullCloudData(showFeedback = false) {
            ...
            try {
                ...
                const res = await fetch(cloudApiUrl + '?action=get_all');
                const json = await res.json();

                if (json && json.status === 'success' && json.store) {
                    // Merge cloud store with local store
                    Object.assign(store, json.store);
                    localStorage.setItem('EXIUM_SWEATER_STORE', JSON.stringify(store));
                ...
                if (showFeedback) alert("Could not connect to Google Drive API. Please check URL or network permissions.");
            }
        }
        ...
        function renderAdminKpisAndSummaries() {
            ...
            ALL_TERRITORIES.forEach(t => {
                ...
                if (d) {
                    ['m1', 'm2', 'm3', 'm4'].forEach(m => {
                        if (d[`c1_${m}_sweater`] && d[`c1_${m}_size`]) c1Sweaters++;
                    });
                    ['d1', 'd2', 'd3', 'd4'].forEach(di => {
                        if (d[`c2_${di}_sweater`] && d[`c2_${di}_size`]) c2Sweaters++;
                    });
                }
            });
        ...
        function deleteSingleRegionData(regCode) {
            const r = REGION_MAP[regCode];
            ...
            alert(`🗑️ All data for ${r.region_name} has been cleared.`);
        }
...
print(f"Successfully generated enhanced {portal_path} and {index_path} ({len(final_html)} bytes)!")
