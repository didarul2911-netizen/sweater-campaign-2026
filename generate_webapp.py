...
###ZONE_OPTIONS###                        </select>
    ...
    <script>
        ...
        function handlePasswordKey(e) {
            ...
            const regSel = document.getElementById('select-region');
            if (regSel) {
                regSel.innerHTML = '<option value="">-- 2. Select Region --</option>';
                regSel.disabled = true;
            }
        ...
        function getTabStyles(status, isActive) {
            if (status === 'Complete') {
                return {
                    badgeText: '✓ Complete',
                    btnClasses: isActive 
                        ? 'bg-emerald-600 text-white border-2 border-emerald-700 shadow-md ring-2 ring-emerald-400/50' 
                        : 'bg-emerald-50/80 hover:bg-emerald-100 border border-emerald-300 text-slate-900 shadow-sm',
                    titleClass: isActive ? 'text-white' : 'text-slate-900',
                    sapClass: isActive ? 'text-emerald-100 font-bold' : 'text-emerald-800 font-bold',
                ...
                return {
                    ...
                    btnClasses: isActive 
                        ? 'bg-amber-500 text-slate-950 border-2 border-amber-600 shadow-md ring-2 ring-amber-400/50' 
                        : 'bg-amber-50/80 hover:bg-amber-100 border border-amber-300 text-slate-900 shadow-sm',
                    titleClass: 'text-slate-950',
                    sapClass: isActive ? 'text-amber-950 font-bold' : 'text-amber-900 font-bold',
                    badgeClasses: isActive 
                        ? 'bg-white text-slate-950 font-black shadow-sm' 
                        : 'bg-amber-100 text-amber-900 border border-amber-300 font-black'
                };
                        } else {
                // Not Started (Grey Background & Black Text when unselected; Black Background & White Text when selected)
                return {
                    badgeText: '○ Not Started',
                    btnClasses: isActive 
                        ? 'bg-black text-white border-2 border-black shadow-md ring-2 ring-slate-400/40' 
                        : 'bg-slate-100 hover:bg-slate-200/80 border border-slate-300 text-slate-900 shadow-sm',
                    titleClass: isActive ? 'text-white' : 'text-slate-900',
                    sapClass: isActive ? 'text-slate-300' : 'text-slate-600',
                    badgeClasses: isActive 
        ...
        function updateActiveTerritoryTabStyle(idx, status) {
            const btn = document.getElementById(`terr-tab-btn-${idx}`);
            if (!btn || !currentRegionCode || !REGION_MAP[currentRegionCode]) return;
            const r = REGION_MAP[currentRegionCode];
            const t = r.territories[idx];
            const styles = getTabStyles(status, true);

            btn.className = `w-full text-left p-2.5 sm:p-3 rounded-2xl text-xs font-bold transition flex items-center justify-between ${styles.btnClasses}`;
            btn.innerHTML = `
                <div class="truncate pr-1 min-w-0">
                    <div class="truncate font-black text-xs ${styles.titleClass}">${t.territory_name}</div>
                    <div class="text-[10px] ${styles.sapClass} font-mono">SAP: ${t.sap_territory_code}</div>
                </div>
                <span class="text-[10px] px-2 py-0.5 rounded-full flex-shrink-0 whitespace-nowrap ${styles.badgeClasses}">${styles.badgeText}</span>
            `;

            // Update mobile option
            const mobSelect = document.getElementById('mobile-territory-select');
            if (mobSelect && mobSelect.options[idx]) {
                mobSelect.options[idx].textContent = `${t.territory_name} (${status})`;
            }

            // Update progress badge
            let completedCount = 0;
            r.territories.forEach(terr => {
                const terrD = store[String(terr.sap_territory_code)] || {};
                if (getTerritoryStatus(terrD) === 'Complete') completedCount++;
            });
            const regBadge = document.getElementById('region-progress-badge');
            if (regBadge) regBadge.textContent = `${completedCount}/${r.territories.length} Done`;
        }
        ...
        function renderTerritoryTabs() {
            ...
            r.territories.forEach((t, idx) => {
                ...
                const styles = getTabStyles(status, isActive);
            ...
            if (!currentRegionCode || !REGION_MAP[currentRegionCode]) return;
            const r = REGION_MAP[currentRegionCode];
            if (idx < 0 || idx >= r.territories.length) return;
            ...
            const titleEl = document.getElementById('current-territory-title');
            if (titleEl) titleEl.textContent = t.territory_name;

            const codeEl = document.getElementById('current-territory-code');
            if (codeEl) codeEl.textContent = `SAP Code: ${terrCode}`;
            ...
            // Campaign 1: 1 Doctor + 3 (or 4) Sweaters
            ...
            // Update indicators and dynamic sweater counter
            updateTerritorySlotCheckBadges(d);
            updateC1DynamicCounter();

            // Re-render sidebar territory tabs so all background colors and selection rings reflect current state
            renderTerritoryTabs();
        ...
        function updateC1DynamicCounter() {
            ...
            if (badgeEl) {
                if (completedSweaters === maxCount && maxCount > 0) {
        ...
        function onDataChanged() {
            if (isRegionLocked() || !currentRegionCode || !REGION_MAP[currentRegionCode]) return;
            const r = REGION_MAP[currentRegionCode];
            const t = r.territories[activeTerritoryIndex];
            const terrCode = String(t.sap_territory_code);
            ...
            // 1. Instant Active Territory Banner update
            const statusBadge = document.getElementById('current-territory-status');
            if (statusBadge) {
                statusBadge.textContent = status;
                statusBadge.className = `text-[10px] sm:text-xs font-black px-3 py-1 rounded-full ${
            ...
            // 2. Instant Left-side Territory item update (0ms, no DOM rebuild)
            updateActiveTerritoryTabStyle(activeTerritoryIndex, status);

            // 3. Instant Dynamic Sweater Counter update
            updateC1DynamicCounter();

            // 4. Update slot check badges
            updateTerritorySlotCheckBadges(terrData);

            // 5. Debounced cloud sync
            if (autoSyncTimeout) clearTimeout(autoSyncTimeout);
            autoSyncTimeout = setTimeout(() => {
        ...
        function saveCurrentTerritoryClick() {
            ...
            // Campaign 1: 1 Doctor (Name + 6-digit RPL) + 3 Mandatory Sweaters (Sweater + Size)
            const c1Mandatory = Boolean(
                d.c1_doc_name && d.c1_doc_name.trim() !== '' && 
            ...
            // If Sweater 4 has either sweater or size selected, both must be complete
            ...
            // Campaign 2: Exactly 3 Doctors (Name + 6-digit RPL + Sweater + Size for each)
            const c2Doc1Ok = Boolean(d.c2_d1_name && d.c2_d1_name.trim() !== '' && d.c2_d1_rpl && String(d.c2_d1_rpl).trim().length === 6 && d.c2_d1_sweater && d.c2_d1_size);
            const c2Doc2Ok = Boolean(d.c2_d2_name && d.c2_d2_name.trim() !== '' && d.c2_d2_rpl && String(d.c2_d2_rpl).trim().length === 6 && d.c2_d2_sweater && d.c2_d2_size);
            const c2Doc3Ok = Boolean(d.c2_d3_name && d.c2_d3_name.trim() !== '' && d.c2_d3_rpl && String(d.c2_d3_rpl).trim().length === 6 && d.c2_d3_sweater && d.c2_d3_size);
            ...
            const hasAny = Boolean(
                (d.c1_doc_name && d.c1_doc_name.trim() !== '') || 
                (d.c1_doc_rpl && String(d.c1_doc_rpl).trim() !== '') || 
                d.c1_m1_sweater || d.c1_m1_size || 
                d.c1_m2_sweater || d.c1_m2_size || 
                d.c1_m3_sweater || d.c1_m3_size || 
                d.c1_m4_sweater || d.c1_m4_size ||
                (d.c2_d1_name && d.c2_d1_name.trim() !== '') || 
                (d.c2_d1_rpl && String(d.c2_d1_rpl).trim() !== '') || 
                d.c2_d1_sweater || d.c2_d1_size ||
                (d.c2_d2_name && d.c2_d2_name.trim() !== '') || 
                (d.c2_d2_rpl && String(d.c2_d2_rpl).trim() !== '') || 
                d.c2_d2_sweater || d.c2_d2_size ||
                (d.c2_d3_name && d.c2_d3_name.trim() !== '') || 
                (d.c2_d3_rpl && String(d.c2_d3_rpl).trim() !== '') || 
                d.c2_d3_sweater || d.c2_d3_size
            );
