...
b64_men_size = get_image_base64(os.path.join(base_dir, "Men's Size.jpeg"), max_dim=1200, quality=90)
b64_fem_size = get_image_base64(os.path.join(base_dir, "Female Size.jpeg"), max_dim=1200, quality=90)

# Pre-render Zone options directly in HTML
...
###ZONE_OPTIONS###                        </select>
    ...
    <div id="admin-modal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm hidden flex items-center justify-center p-3 sm:p-6" onclick="closeAdminModal()">
        <div class="bg-white border border-slate-200 rounded-3xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col overflow-hidden" onclick="event.stopPropagation()">
            <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-900 text-white">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-xl bg-orange-500 text-slate-950 flex items-center justify-center font-black text-sm shadow">
                        <i class="fa-solid fa-shield-halved"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-xs sm:text-sm">Central Admin Control & Master Export</h3>
                        <p class="text-[10px] text-slate-400">Total 252 Regions &bull; 1,856 Territories</p>
                    </div>
                </div>
            ...
            <div class="p-4 sm:p-6 overflow-y-auto custom-scrollbar space-y-6 flex-1">
                <!-- Auth Box -->
                <div id="admin-auth-view" class="max-w-sm mx-auto py-8 text-center space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Enter master admin password</p>
                    </div>
                    <div class="space-y-2">
                ...
                <div id="admin-dashboard-view" class="hidden space-y-6">
                    <div class="flex flex-wrap items-center justify-between gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-bold text-slate-700">Submissions Access:</span>
                            <button id="toggle-global-access-btn" onclick="toggleGlobalSubmissionsAccess()" class="px-3 py-1.5 bg-emerald-600 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 shadow-sm">
                                <i class="fa-solid fa-lock-open"></i> <span>Open</span>
                            </button>
                        </div>
                        <div class="flex items-center gap-2">
                            <button onclick="exportMasterExcelFromAdmin()" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-600/20 transition active:scale-95">
                                <i class="fa-solid fa-file-excel text-sm"></i>
                                <span>Export Master Excel (.xlsx)</span>
                            </button>
                            <button onclick="logoutAdmin()" class="px-3.5 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 text-xs font-bold rounded-xl transition">
                            ...
                            <table class="w-full text-xs text-center border-collapse">
                                <thead class="bg-slate-900 text-white text-[11px] font-bold">
                                    <tr>
                                        ...
                                        <th class="p-2.5 bg-orange-600">Total</th>
                                    </tr>
                                </thead>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-700">All 252 Regions Status</h4>
                            <input type="text" id="admin-region-search" oninput="filterAdminRegions(this.value)" placeholder="Search Region or RH..." class="bg-slate-50 border border-slate-300 rounded-xl px-3 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-orange-500 w-64">
                        </div>
                        <div class="overflow-x-auto bg-white border border-slate-200 rounded-2xl shadow-sm max-h-[360px] overflow-y-auto custom-scrollbar">
                            <table class="w-full text-xs text-left border-collapse">
                                <thead class="bg-slate-100 text-slate-600 uppercase text-[10px] sticky top-0 border-b border-slate-200">
                                    <tr>
                                        <th class="p-2.5">SAP Code</th><th class="p-2.5">Region</th><th class="p-2.5">Zone</th><th class="p-2.5">Regional Head</th><th class="p-2.5">Progress</th><th class="p-2.5">Lock State</th><th class="p-2.5 text-right">Actions</th>
                                    </tr>
                                </thead>
    ...
    <script>
        ...
        function populateZoneDropdown() {
            ...
            if (!sel) return;
            // If already has options, ensure value
        ...
        function showAdminDashboard() {
            ...
            if (isGlobalAccessOpen) {
                ...
                accessBtn.innerHTML = '<i class="fa-solid fa-lock-open"></i> <span>Open</span>';
            } else {
                ...
                accessBtn.innerHTML = '<i class="fa-solid fa-lock"></i> <span>Locked</span>';
            }
...
print(f"Successfully generated {portal_path} and {index_path} ({len(final_html)} bytes)!")
